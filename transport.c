/* Team: Dai Nguyen, Olsen Ong, Irwan Winarto
 * transport.c
 *
...
// ------------------------------- Olsen Part ------------------------------ //
STCPHeader* create_DATA_packet(unsigned int seq,
                               unsigned int ack,
                               char* payload,
                               size_t payload_length);

bool send_DATA_packet_network(mysocket_t sd,
                              context_t* ctx,
                              char* payload,
                              size_t payload_length);

void send_DATA_packet_app(mysocket_t sd,
                          context_t* ctx,
                          char* payload,
                          size_t length);

void parse_DATA_packet(context_t* ctx,
                       char* payload,
                       bool& isFIN,
                       bool& isDUP);
...
static void control_loop(mysocket_t sd, context_t *ctx)
{
  ...
  while (!ctx->done)
  {
    ...
    if (event & APP_DATA)
    {
      ...
      (STCP_MSS < ctx->rec_wind_size ? STCP_MSS : ctx->rec_wind_size) -
      sizeof(STCPHeader);
...
STCPHeader *create_SYN_Packet(unsigned int seq, unsigned int ack)
{
  ...
  SYN_packet->th_off = htons(STCP_HEADER_LEN);    // header size offset
...
STCPHeader *create_SYN_ACK_Packet(unsigned int seq, unsigned int ack)
{
  ...
  SYN_ACK_packet->th_off = htons(STCP_HEADER_LEN);  // header size offset
  SYN_ACK_packet->th_flags = (TH_SYN | TH_ACK);     // set packet type SYN_ACK
...
STCPHeader *create_ACK_Packet(unsigned int seq, unsigned int ack)
{
  ...
  ACK_packet->th_off = htons(STCP_HEADER_LEN);  // header size offset
...
bool send_SYNACK(mysocket_t sd, context_t *ctx)
{
  ...
  ssize_t bytes_sent;
  if ((bytes_sent =
       stcp_network_send(sd, SYNACK_pkt, sizeof(STCPHeader), NULL)) <= 0)
  {
...
void wait4_ACK(mysocket_t sd, context_t *ctx)
{
  ...
  ssize_t bytes_recv;
  if((bytes_recv =
      stcp_network_recv(sd, buffer, STCP_MSS)) < sizeof(STCPHeader))
  {
...
void send_DATA_packet_app(mysocket_t sd, context_t *ctx,
                          char *payload, size_t length)
{
...
bool send_DATA_packet_network(mysocket_t sd, context_t *ctx,
                              char *payload, size_t payload_length)
{
  
  STCPHeader *DATA_packet = create_DATA_packet(ctx->snd_seq_num,
                                               ctx->rec_seq_num + 1, payload,
                                               payload_length);
  ...
  ssize_t sentBytes = stcp_network_send(sd, DATA_packet,
                                        sizeof(STCPHeader) + payload_length,
                                        NULL);
...
bool send_FIN_packet(mysocket_t sd, context_t* ctx) {
  ...
  STCPHeader* FIN_packet = create_FIN_packet(ctx->snd_seq_num,
                                             ctx->rec_seq_num + 1);
  
  ctx->snd_seq_num++;
  
  // Send FIN packet
  ssize_t sentBytes = stcp_network_send(sd, FIN_packet,
                                        sizeof(STCPHeader), NULL);
